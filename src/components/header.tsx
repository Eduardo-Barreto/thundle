...
export function Header({ puzzleNumber, onOpenPuzzles, onOpenStats }: HeaderProps) {
  ...
  return (
    <header className="relative flex items-baseline justify-center gap-4 px-6 pt-6 pb-7">
      <h1 className="font-mono text-[28px] font-bold tracking-tight">
        thundle
...
function IconButton({
  ...
  return (
    <button
      ...
      title={label}
      onClick={onClick}
      className="text-t3 hover:text-t2 focus-visible:outline-thunder-yellow flex size-8 cursor-pointer items-center justify-center rounded-md border border-white/6 transition-all duration-160 ease-[cubic-bezier(0.23,1,0.32,1)] hover:border-white/10 focus-visible:outline-2 focus-visible:outline-offset-2 active:scale-93"
    >
      <svg
        className="size-3.5"
        viewBox="0 0 24 24"
